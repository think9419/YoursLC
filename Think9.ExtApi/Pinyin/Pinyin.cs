...
namespace Think9.NPinyin
{
    public static class Pinyin
    {
        ...
        public static string GetPinyin(string text, bool bSpace)
        {
            string strSpace = "";
            if (bSpace)
            {
            ...
            for (var i = 0; i < text.Length; ++i)
            {
                string py = GetPinyin(text[i]).Trim();
                if (py != "")
                {
                    sbPinyin.Append(py.Substring(0, 1).ToUpper() + py.Substring(1, py.Length - 1) + strSpace);
                    sbPinyin.Append("");
                }
            }
