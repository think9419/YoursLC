	...
	var api = {
        ...
        disableGridColumnFormControl: function (grid, strDisable) {
			if (grid =='tb_SJHXZB') {
				...
				if (strDisable.indexOf('[v4]') > -1) {
					$('#v4').attr('disabled', 'disabled');//填报人
					$('#v4').removeAttr('lay-verify');//填报人
				...
				if (strDisable.indexOf('[v6]') > -1) {
					$('#v6').attr('disabled', 'disabled');//隐藏
					$('#v6').removeAttr('lay-verify');//隐藏
		...
		getControlListFrmGridColumn: function (grid) {
			...
			if (grid =='tb_SJHXZB') {
				...
				_list.push({ ControlID: 'v4', ControlValue: $('#v4').val(), ControlType: '1' });//填报人
				_list.push({ ControlID: 'v5', ControlValue: $('#v5').val(), ControlType: '1' });//备注
				_list.push({ ControlID: 'v6', ControlValue: $('#v6').val(), ControlType: '1' });//隐藏
			}
			return _list;
		},
		...
		setValueByList: function (lists, _tbid, _rowid) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//
                ...
                for (var i = 0; i < trList.length; i++) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_SJHXZB') {
						for (var item in lists) {
		...
		getValueFromPopUp: function (_tbid, _indexid, _id, _v, _value) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//主表或者子表纵列
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if (flag.startsWith(_starts) && _tbid == 'tb_SJHXZB') {
		...
		getCurrentGridTable: function (tbid, id) {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_SJHXZB#' + id + '#') && tbid =='tb_SJHXZB') {
					...
					_list.push({ ControlID: 'v4', ControlValue: tdArr.eq(5).find('input').val(), ControlType: '1' });//填报人
		...
		disabledGridTable: function (grid) {
			//控制子表工具栏按钮
			if (grid == 'tb_SJHXZB') {
				$('#del_tb_SJHXZB').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_SJHXZB').removeAttr('lay-event');
				$('#add_tb_SJHXZB').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_SJHXZB').removeAttr('lay-event');
				$('#edit_tb_SJHXZB').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_SJHXZB').removeAttr('lay-event');
				$('#add_tb_SJHXZB').attr('disabled', 'true');
				$('#edit_tb_SJHXZB').attr('disabled', 'true');
				$('#del_tb_SJHXZB').attr('disabled', 'true');
			}
			var trList = $(".layui-table").find("tr");  //获取table下的所有tr
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_SJHXZB#') && grid =='tb_SJHXZB') {
					...
					tdArr.eq(5).find('input').attr('disabled', 'disabled');//填报人
