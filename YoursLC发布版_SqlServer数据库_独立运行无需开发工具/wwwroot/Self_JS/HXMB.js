/**
 * Create:admin 2024-09-24 13:11:05
...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
		...
		disabledControls: function (prcno, _fields, _hiddens) {
			...
			//上报日期
			if (_fields != '#all#' && _fields.indexOf(',inReportDate,') == -1) {
				$('#inReportDate').attr('disabled','disabled');//禁用
				$('#inReportDate').attr('class','layui-input layui-disabled');
				$('#inReportDate').removeAttr('lay-verify');//去除校验
				$('#select_inReportDate').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inReportDate;') != -1) {
					$('#inReportDate').attr('placeholder','******');//保密
					$('#inReportDate').val('');
				}
			}
			//上报人
			if (_fields != '#all#' && _fields.indexOf(',inReportPerson,') == -1) {
				$('#inReportPerson').attr('disabled','disabled');//禁用
				$('#inReportPerson').attr('class','layui-input layui-disabled');
				$('#inReportPerson').removeAttr('lay-verify');//去除校验
				$('#select_inReportPerson').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inReportPerson;') != -1) {
					$('#inReportPerson').attr('placeholder','******');//保密
					$('#inReportPerson').val('');
				}
			}
			//小数
			if (_fields != '#all#' && _fields.indexOf(',inDecimal,') == -1) {
				$('#inDecimal').attr('disabled','disabled');//禁用
				$('#inDecimal').attr('class','layui-input layui-disabled');
				$('#inDecimal').removeAttr('lay-verify');//去除校验
				$('#select_inDecimal').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inDecimal;') != -1) {
					$('#inDecimal').attr('placeholder','******');//保密
					$('#inDecimal').val('');
				}
			}
			//修改时间
			if (_fields != '#all#' && _fields.indexOf(',inChangeTime,') == -1) {
				$('#inChangeTime').attr('disabled','disabled');//禁用
				$('#inChangeTime').attr('class','layui-input layui-disabled');
				$('#inChangeTime').removeAttr('lay-verify');//去除校验
				$('#select_inChangeTime').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inChangeTime;') != -1) {
					$('#inChangeTime').attr('placeholder','******');//保密
					$('#inChangeTime').val('');
				}
			}
			//备注
		...
		getControlValueList: function () {
			...
			_list.push({ ControlID: 'inReportDate', ControlValue: $('#inReportDate').val(), ControlType: '1' });//上报日期
			_list.push({ ControlID: 'inReportPerson', ControlValue: $('#inReportPerson').val(), ControlType: '1' });//上报人
			_list.push({ ControlID: 'inDecimal', ControlValue: $('#inDecimal').val(), ControlType: '1' });//小数
			_list.push({ ControlID: 'inChangeTime', ControlValue: $('#inChangeTime').val(), ControlType: '1' });//修改时间
		...
		getControlValueListFrmParent: function () {
		    var _list = [];
			_list.push({ ControlID: 'listid', ControlValue: $('#_listid').val(), ControlType: '1' });
			_list.push({ ControlID: 'inReportDate', ControlValue: $(window.parent.document).find('#inReportDate').val(), ControlType: '1' });//上报日期
			_list.push({ ControlID: 'inReportPerson', ControlValue: $(window.parent.document).find('#inReportPerson').val(), ControlType: '1' });//上报人
			_list.push({ ControlID: 'inDecimal', ControlValue: $(window.parent.document).find('#inDecimal').val(), ControlType: '1' });//小数
			_list.push({ ControlID: 'inChangeTime', ControlValue: $(window.parent.document).find('#inChangeTime').val(), ControlType: '1' });//修改时间
		...
		getControlListFrmGridColumn: function (grid) {
			...
			if (grid =='tb_HXMBZB') {
				...
				_list.push({ ControlID: 'v3', ControlValue: $('#v3').val(), ControlType: '1' });//小数
				_list.push({ ControlID: 'v4', ControlValue: $('#v4').val(), ControlType: '1' });//上报人
				_list.push({ ControlID: 'v5', ControlValue: $('#v5').val(), ControlType: '1' });//备注
			}
			return _list;
		},
		...
		setValueByList: function (lists, _tbid, _rowid) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//
                ...
                for (var i = 0; i < trList.length; i++) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_HXMBZB') {
						for (var item in lists) {
		...
		getValueFromPopUp: function (_tbid, _indexid, _id, _v, _value) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//主表或者子表纵列
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if (flag.startsWith(_starts) && _tbid == 'tb_HXMBZB') {
		...
		getFirstGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				...
				if (flag.startsWith('#tb_HXMBZB#') && flag.substr(-3) == '#0#') {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//编码
					_row.v2 = tdArr.eq(3).find('input').val();//名称
					_row.v3 = tdArr.eq(4).find('input').val();//小数
					_row.v4 = tdArr.eq(5).find('input').val();//上报人
					_row.v5 = tdArr.eq(6).find('input').val();//备注
					_list.push(_row);
				}
			}
			return _list;
		...
		getCurrentGridTable: function (tbid, id) {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_HXMBZB#' + id + '#') && tbid =='tb_HXMBZB') {
					_list.push({ ControlID: 'v1', ControlValue: tdArr.eq(2).find('input').val(), ControlType: '1' });//编码
					_list.push({ ControlID: 'v2', ControlValue: tdArr.eq(3).find('input').val(), ControlType: '1' });//名称
					_list.push({ ControlID: 'v3', ControlValue: tdArr.eq(4).find('input').val(), ControlType: '1' });//小数
					_list.push({ ControlID: 'v4', ControlValue: tdArr.eq(5).find('input').val(), ControlType: '1' });//上报人
					_list.push({ ControlID: 'v5', ControlValue: tdArr.eq(6).find('input').val(), ControlType: '1' });//备注
				}
			}
			return _list;
		},
		...
		disabledGridTable: function (grid) {
			//控制子表工具栏按钮
			if (grid == 'tb_HXMBZB') {
				$('#del_tb_HXMBZB').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_HXMBZB').removeAttr('lay-event');
				$('#add_tb_HXMBZB').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_HXMBZB').removeAttr('lay-event');
				$('#edit_tb_HXMBZB').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_HXMBZB').removeAttr('lay-event');
				$('#add_tb_HXMBZB').attr('disabled', 'true');
				$('#edit_tb_HXMBZB').attr('disabled', 'true');
				$('#del_tb_HXMBZB').attr('disabled', 'true');
			}
			var trList = $(".layui-table").find("tr");  //获取table下的所有tr
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_HXMBZB#') && grid =='tb_HXMBZB') {
					tdArr.eq(2).find('input').attr('disabled', 'disabled');//编码
					tdArr.eq(2).find('button').unbind('click');//编码
					tdArr.eq(3).find('input').attr('disabled', 'disabled');//名称
					tdArr.eq(4).find('input').attr('disabled', 'disabled');//小数
					tdArr.eq(5).find('input').attr('disabled', 'disabled');//上报人
					tdArr.eq(6).find('input').attr('disabled', 'disabled');//备注
