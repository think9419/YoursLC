...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
        ...
        disableGridColumnFormControl: function (grid, strDisable) {
			if (grid =='tb_A2SubTable') {
				...
				if (strDisable.indexOf('[v4]') > -1) {
					$('#v4').attr('disabled', 'disabled');//备注
					$('#v4').removeAttr('lay-verify');//备注
		...
		getControlListFrmGridColumn: function (grid) {
			...
			if (grid =='tb_A2SubTable') {
				...
				_list.push({ ControlID: 'v4', ControlValue: $('#v4').val(), ControlType: '1' });//备注
			}
			return _list;
		},
		...
		setValueByList: function (lists, _tbid, _rowid) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//
                ...
                for (var i = 0; i < trList.length; i++) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_A2SubTable') {
						for (var item in lists) {
		...
		getValueFromPopUp: function (_tbid, _indexid, _id, _v, _value) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//主表或者子表纵列
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if (flag.startsWith(_starts) && _tbid == 'tb_A2SubTable') {
		...
		getCurrentGridTable: function (tbid, id) {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_A2SubTable#' + id + '#') && tbid =='tb_A2SubTable') {
					...
					_list.push({ ControlID: 'v4', ControlValue: tdArr.eq(4).find('input').val(), ControlType: '1' });//备注
				}
			}
			return _list;
		},
		...
		foreachGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_A2SubTable#')) {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(1).find('input').val();//编码
					_row.v2 = tdArr.eq(2).find('input').val();//名称
		...
		disabledGridTable: function (grid) {
			//控制子表工具栏按钮
			if (grid == 'tb_A2SubTable') {
				$('#del_tb_A2SubTable').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_A2SubTable').removeAttr('lay-event');
				$('#add_tb_A2SubTable').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_A2SubTable').removeAttr('lay-event');
				$('#edit_tb_A2SubTable').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_A2SubTable').removeAttr('lay-event');
				$('#add_tb_A2SubTable').attr('disabled', 'true');
				$('#edit_tb_A2SubTable').attr('disabled', 'true');
				$('#del_tb_A2SubTable').attr('disabled', 'true');
			}
			var trList = $(".layui-table").find("tr");  //获取table下的所有tr
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_A2SubTable#') && grid =='tb_A2SubTable') {
					...
					tdArr.eq(4).find('input').attr('disabled', 'disabled');//备注
	...
	//暴露接口
	exports('A2WriteBackTheTarget', api);
});