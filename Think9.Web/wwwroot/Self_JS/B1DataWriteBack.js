	...
	var api = {
		...
		setValueByList: function (lists, _tbid, _rowid) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//
                ...
                for (var i = 0; i < trList.length; i++) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_B1SubTable') {
						for (var item in lists) {
		...
		getValueFromPopUp: function (_tbid, _indexid, _id, _v, _value) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//主表或者子表纵列
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if (flag.startsWith(_starts) && _tbid == 'tb_B1SubTable') {
		...
		getFirstGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				...
				if (flag.startsWith('#tb_B1SubTable#') && flag.substr(-3) == '#0#') {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//编码
					_row.v2 = tdArr.eq(3).find('input').val();//名称
					_row.v3 = tdArr.eq(4).find('input').val();//数量
					_list.push(_row);
				}
			}
			return _list;
		...
		getCurrentGridTable: function (tbid, id) {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_B1SubTable#' + id + '#') && tbid =='tb_B1SubTable') {
					...
					_list.push({ ControlID: 'v3', ControlValue: tdArr.eq(4).find('input').val(), ControlType: '1' });//数量
				}
			}
			return _list;
		},
		...
		foreachGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_B1SubTable#')) {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//编码
					_row.v2 = tdArr.eq(3).find('input').val();//名称
		...
		disabledGridTable: function (grid) {
			//控制子表工具栏按钮
			if (grid == 'tb_B1SubTable') {
				$('#del_tb_B1SubTable').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_B1SubTable').removeAttr('lay-event');
				$('#add_tb_B1SubTable').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_B1SubTable').removeAttr('lay-event');
				$('#edit_tb_B1SubTable').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_B1SubTable').removeAttr('lay-event');
				$('#add_tb_B1SubTable').attr('disabled', 'true');
				$('#edit_tb_B1SubTable').attr('disabled', 'true');
				$('#del_tb_B1SubTable').attr('disabled', 'true');
			}
			var trList = $(".layui-table").find("tr");  //获取table下的所有tr
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_B1SubTable#') && grid =='tb_B1SubTable') {
					...
					tdArr.eq(4).find('input').attr('disabled', 'disabled');//数量
