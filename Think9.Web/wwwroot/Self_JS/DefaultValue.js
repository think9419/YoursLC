...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
		...
		disabledControls: function (prcno, _fields, _hiddens) {
			...
			//隐藏并赋值
			if (_fields != '#all#' && _fields.indexOf(',inHideAndAssignValues,') == -1) {
				$('#inHideAndAssignValues').attr('disabled','disabled');//禁用
				$('#inHideAndAssignValues').attr('class','layui-input layui-disabled');
				$('#inHideAndAssignValues').removeAttr('lay-verify');//去除校验
				$('#select_inHideAndAssignValues').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inHideAndAssignValues;') != -1) {
					$('#inHideAndAssignValues').attr('placeholder','******');//保密
					$('#inHideAndAssignValues').val('');
				}
			}
			//文本框
			if (_fields != '#all#' && _fields.indexOf(',inTextBox,') == -1) {
				$('#inTextBox').attr('disabled','disabled');//禁用
				$('#inTextBox').attr('class','layui-input layui-disabled');
				$('#inTextBox').removeAttr('lay-verify');//去除校验
				$('#select_inTextBox').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inTextBox;') != -1) {
					$('#inTextBox').attr('placeholder','******');//保密
					$('#inTextBox').val('');
				}
			}
			//备注
		...
		getControlValueList: function () {
			...
			_list.push({ ControlID: 'inHideAndAssignValues', ControlValue: $('#inHideAndAssignValues').val(), ControlType: '1' });//隐藏并赋值
			_list.push({ ControlID: 'inTextBox', ControlValue: $('#inTextBox').val(), ControlType: '1' });//文本框
		...
		getControlValueListFrmParent: function () {
		    var _list = [];
			_list.push({ ControlID: 'listid', ControlValue: $('#_listid').val(), ControlType: '1' });
			_list.push({ ControlID: 'inHideAndAssignValues', ControlValue: $(window.parent.document).find('#inHideAndAssignValues').val(), ControlType: '1' });//隐藏并赋值
			_list.push({ ControlID: 'inTextBox', ControlValue: $(window.parent.document).find('#inTextBox').val(), ControlType: '1' });//文本框
