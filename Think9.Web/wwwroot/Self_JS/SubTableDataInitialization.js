...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
        ...
        disableGridColumnFormControl: function (grid, strDisable) {
			if (grid =='tb_Subtable51') {
				if (strDisable.indexOf('[v1]') > -1) {
					$('#v1').attr('disabled', 'disabled');//列1
					$('#v1').removeAttr('lay-verify');//列1
			...
			if (grid =='tb_Subtable52') {
				...
				if (strDisable.indexOf('[v3]') > -1) {
					$('#v3').attr('disabled', 'disabled');//数值
					$('#v3').removeAttr('lay-verify');//数值
		...
		getControlListFrmGridColumn: function (grid) {
			...
			_list.push({ ControlID: 'listid', ControlValue: $('#_listid').val(), ControlType: '1' });
			if (grid =='tb_Subtable51') {
				_list.push({ ControlID: 'v1', ControlValue: $('#v1').val(), ControlType: '1' });//列1
			...
			if (grid =='tb_Subtable52') {
				...
				_list.push({ ControlID: 'v3', ControlValue: $('#v3').val(), ControlType: '1' });//数值
			}
			return _list;
		},
		...
		setValueByList: function (lists, _tbid, _rowid) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//
                ...
                for (var i = 0; i < trList.length; i++) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_Subtable51') {
						for (var item in lists) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_Subtable52') {
						for (var item in lists) {
							var _list = lists[item].ListValue;
							var _value = lists[item].ControlValue;
							var _id = lists[item].ControlID.replace("v", "");//列序号如1、2、3...
		...
		getValueFromPopUp: function (_tbid, _indexid, _id, _v, _value) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//主表或者子表纵列
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if (flag.startsWith(_starts) && _tbid == 'tb_Subtable51') {
						tdArr.eq(parseInt(_v) + 2).find('input').val(_value);
					}
					if (flag.startsWith(_starts) && _tbid == 'tb_Subtable52') {
		...
		getFirstGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				...
				if (flag.startsWith('#tb_Subtable51#') && flag.substr(-3) == '#0#') {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(3).find('input').val();//列1
					_row.v2 = tdArr.eq(4).find('input').val();//列2
					_list.push(_row);
				}
				if (flag.startsWith('#tb_Subtable52#') && flag.substr(-3) == '#0#') {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//编码
					_row.v2 = tdArr.eq(3).find('input').val();//名称
					_row.v3 = tdArr.eq(4).find('input').val();//数值
					_list.push(_row);
				}
			}
			return _list;
		...
		getCurrentGridTable: function (tbid, id) {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable51#' + id + '#') && tbid =='tb_Subtable51') {
					_list.push({ ControlID: 'v1', ControlValue: tdArr.eq(3).find('input').val(), ControlType: '1' });//列1
					_list.push({ ControlID: 'v2', ControlValue: tdArr.eq(4).find('input').val(), ControlType: '1' });//列2
				}
				if (flag.startsWith('#tb_Subtable52#' + id + '#') && tbid =='tb_Subtable52') {
					...
					_list.push({ ControlID: 'v3', ControlValue: tdArr.eq(4).find('input').val(), ControlType: '1' });//数值
				}
			}
			return _list;
		},
		...
		foreachGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable51#')) {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(3).find('input').val();//列1
					_row.v2 = tdArr.eq(4).find('input').val();//列2
				...
				if (flag.startsWith('#tb_Subtable52#')) {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//编码
					_row.v2 = tdArr.eq(3).find('input').val();//名称
		...
		disabledGridTable: function (grid) {
			//控制子表工具栏按钮
			if (grid == 'tb_Subtable51') {
				$('#del_tb_Subtable51').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_Subtable51').removeAttr('lay-event');
				$('#add_tb_Subtable51').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_Subtable51').removeAttr('lay-event');
				$('#edit_tb_Subtable51').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_Subtable51').removeAttr('lay-event');
				$('#add_tb_Subtable51').attr('disabled', 'true');
				$('#edit_tb_Subtable51').attr('disabled', 'true');
				$('#del_tb_Subtable51').attr('disabled', 'true');
			}
			//控制子表工具栏按钮
			if (grid == 'tb_Subtable52') {
				$('#del_tb_Subtable52').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_Subtable52').removeAttr('lay-event');
				$('#add_tb_Subtable52').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_Subtable52').removeAttr('lay-event');
				$('#edit_tb_Subtable52').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_Subtable52').removeAttr('lay-event');
				$('#add_tb_Subtable52').attr('disabled', 'true');
				$('#edit_tb_Subtable52').attr('disabled', 'true');
				$('#del_tb_Subtable52').attr('disabled', 'true');
			}
			var trList = $(".layui-table").find("tr");  //获取table下的所有tr
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable51#') && grid =='tb_Subtable51') {
					tdArr.eq(3).find('input').attr('disabled', 'disabled');//列1
					tdArr.eq(4).find('input').attr('disabled', 'disabled');//列2
				...
				if (flag.startsWith('#tb_Subtable52#') && grid =='tb_Subtable52') {
					tdArr.eq(2).find('input').attr('disabled', 'disabled');//编码
					tdArr.eq(3).find('input').attr('disabled', 'disabled');//名称
					tdArr.eq(4).find('input').attr('disabled', 'disabled');//数值
					tdArr.eq(5).find('button').attr('disabled', 'disabled');//右侧添加|删除按钮
					tdArr.eq(5).find('button').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
					tdArr.eq(5).find('button').removeAttr('lay-event');
				}
	...
	//暴露接口
	exports('SubTableDataInitialization', api);
});