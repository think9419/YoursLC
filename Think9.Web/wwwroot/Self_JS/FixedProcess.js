...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
		...
		disabledControls: function (prcno, _fields, _hiddens) {
			//上报 步骤编码new 序号1 
			if (prcno == 'new') {
				$('#inName').attr('disabled','disabled');//名称
				$('#inName').attr('class','layui-input layui-disabled');//名称
				$('#inName').removeAttr('lay-verify');//名称
				$('#select_inName').unbind('click');//名称 解除点击选择
			}
			//审核 步骤编码ToExamine 序号2 
			if (prcno == 'ToExamine') {
				$('#inDate').attr('disabled','disabled');//日期
				$('#inDate').attr('class','layui-input layui-disabled');//日期
				$('#inDate').removeAttr('lay-verify');//日期
				$('#select_inDate').unbind('click');//日期 解除点击选择
				$('#inDate').attr('placeholder','******');//
				$('#inDate').val('');//日期 保密字段
			}
		},
		...
		setValueByList: function (lists, _tbid, _rowid) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//
                ...
                for (var i = 0; i < trList.length; i++) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_Subtable01') {
						for (var item in lists) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_Subtable02') {
						for (var item in lists) {
							var _list = lists[item].ListValue;
							var _value = lists[item].ControlValue;
							var _id = lists[item].ControlID.replace("v", "");//列序号如1、2、3...
		...
		getValueFromPopUp: function (_tbid, _indexid, _id, _v, _value) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//主表或者子表纵列
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if (flag.startsWith(_starts) && _tbid == 'tb_Subtable01') {
						tdArr.eq(parseInt(_v) + 1).find('input').val(_value);
					}
					if (flag.startsWith(_starts) && _tbid == 'tb_Subtable02') {
						tdArr.eq(parseInt(_v) + 1).find('input').val(_value);
					}
				}
			}
		...
		getFirstGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				...
				if (flag.startsWith('#tb_Subtable01#') && flag.substr(-3) == '#0#') {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//列1
					_row.v2 = tdArr.eq(3).find('input').val();//列2
					_row.v3 = tdArr.eq(4).find('input').val();//列3
					_list.push(_row);
				}
				if (flag.startsWith('#tb_Subtable02#') && flag.substr(-3) == '#0#') {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//列1
					_row.v2 = tdArr.eq(3).find('input').val();//列2
		...
		getCurrentGridTable: function (tbid, id) {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable01#' + id + '#') && tbid =='tb_Subtable01') {
					_list.push({ ControlID: 'v1', ControlValue: tdArr.eq(2).find('input').val(), ControlType: '1' });//列1
				...
				if (flag.startsWith('#tb_Subtable02#' + id + '#') && tbid =='tb_Subtable02') {
					_list.push({ ControlID: 'v1', ControlValue: tdArr.eq(2).find('input').val(), ControlType: '1' });//列1
					_list.push({ ControlID: 'v2', ControlValue: tdArr.eq(3).find('input').val(), ControlType: '1' });//列2
					_list.push({ ControlID: 'v3', ControlValue: tdArr.eq(4).find('input').val(), ControlType: '1' });//列3
				}
		...
		foreachGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable01#')) {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//列1
					_row.v2 = tdArr.eq(3).find('input').val();//列2
				...
				if (flag.startsWith('#tb_Subtable02#')) {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//列1
					_row.v2 = tdArr.eq(3).find('input').val();//列2
		...
		disabledGridTable: function (grid) {
			//控制子表工具栏按钮
			if (grid == 'tb_Subtable01') {
				$('#del_tb_Subtable01').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_Subtable01').removeAttr('lay-event');
				$('#add_tb_Subtable01').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_Subtable01').removeAttr('lay-event');
				$('#edit_tb_Subtable01').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_Subtable01').removeAttr('lay-event');
				$('#add_tb_Subtable01').attr('disabled', 'true');
				$('#edit_tb_Subtable01').attr('disabled', 'true');
				$('#del_tb_Subtable01').attr('disabled', 'true');
			}
			//控制子表工具栏按钮
			if (grid == 'tb_Subtable02') {
				$('#del_tb_Subtable02').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_Subtable02').removeAttr('lay-event');
				$('#add_tb_Subtable02').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_Subtable02').removeAttr('lay-event');
				$('#edit_tb_Subtable02').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_Subtable02').removeAttr('lay-event');
				$('#add_tb_Subtable02').attr('disabled', 'true');
				$('#edit_tb_Subtable02').attr('disabled', 'true');
				$('#del_tb_Subtable02').attr('disabled', 'true');
			}
			var trList = $(".layui-table").find("tr");  //获取table下的所有tr
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable01#') && grid =='tb_Subtable01') {
					tdArr.eq(2).find('input').attr('disabled', 'disabled');//列1
				...
				if (flag.startsWith('#tb_Subtable02#') && grid =='tb_Subtable02') {
					tdArr.eq(2).find('input').attr('disabled', 'disabled');//列1
					tdArr.eq(3).find('input').attr('disabled', 'disabled');//列2
					tdArr.eq(4).find('input').attr('disabled', 'disabled');//列3
					tdArr.eq(5).find('button').attr('disabled', 'disabled');//右侧添加|删除按钮
