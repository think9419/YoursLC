...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
		...
		disabledControls: function (prcno, _fields, _hiddens) {
			...
			//编号
			if (_fields != '#all#' && _fields.indexOf(',inNumber,') == -1) {
				$('#inNumber').attr('disabled','disabled');//禁用
				$('#inNumber').attr('class','layui-input layui-disabled');
				$('#inNumber').removeAttr('lay-verify');//去除校验
				$('#select_inNumber').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inNumber;') != -1) {
					$('#inNumber').attr('placeholder','******');//保密
					$('#inNumber').val('');
				}
			}
			//上报日期
			...
			//填报人
			if (_fields != '#all#' && _fields.indexOf(',inApplicant,') == -1) {
				$('#inApplicant').attr('disabled','disabled');//禁用
				$('#inApplicant').attr('class','layui-input layui-disabled');
				$('#inApplicant').removeAttr('lay-verify');//去除校验
				$('#select_inApplicant').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inApplicant;') != -1) {
					$('#inApplicant').attr('placeholder','******');//保密
					$('#inApplicant').val('');
				}
			}
		},
        ...
        disableListBtn: function (strDisable) {
			...
			if (strDisable == 'all' || strDisable.indexOf('[btn2_SetEmpty]') > -1) {
				$('#btn2_SetEmpty').attr('class', 'layui-btn layui-btn-disabled layui-btn-sm');//禁用置空
				$('#btn2_SetEmpty').removeAttr('lay-event');
			}
		},
        ...
        disableGridColumnFormControl: function (grid, strDisable) {
			if (grid =='tb_B2SubTable') {
				...
				if (strDisable.indexOf('[v3]') > -1) {
					$('#v3').attr('disabled', 'disabled');//数量
					$('#v3').removeAttr('lay-verify');//数量
		...
		getControlValueList: function () {
			...
			_list.push({ ControlID: 'inNumber', ControlValue: $('#inNumber').val(), ControlType: '1' });//编号
			_list.push({ ControlID: 'inReportDate', ControlValue: $('#inReportDate').val(), ControlType: '1' });//上报日期
			_list.push({ ControlID: 'inApplicant', ControlValue: $('#inApplicant').val(), ControlType: '1' });//填报人
			return _list;
		},
		...
		getControlValueListFrmParent: function () {
		    var _list = [];
			_list.push({ ControlID: 'listid', ControlValue: $('#_listid').val(), ControlType: '1' });
			_list.push({ ControlID: 'inNumber', ControlValue: $(window.parent.document).find('#inNumber').val(), ControlType: '1' });//编号
			_list.push({ ControlID: 'inReportDate', ControlValue: $(window.parent.document).find('#inReportDate').val(), ControlType: '1' });//上报日期
			_list.push({ ControlID: 'inApplicant', ControlValue: $(window.parent.document).find('#inApplicant').val(), ControlType: '1' });//填报人
			return _list;
		},
		...
		getControlListFrmGridColumn: function (grid) {
			...
			if (grid =='tb_B2SubTable') {
				...
				_list.push({ ControlID: 'v3', ControlValue: $('#v3').val(), ControlType: '1' });//数量
			}
			return _list;
		},
		...
		setValueByList: function (lists, _tbid, _rowid) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//
                ...
                for (var i = 0; i < trList.length; i++) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_B2SubTable') {
						for (var item in lists) {
		...
		getValueFromPopUp: function (_tbid, _indexid, _id, _v, _value) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//主表或者子表纵列
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if (flag.startsWith(_starts) && _tbid == 'tb_B2SubTable') {
		...
		getFirstGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				...
				if (flag.startsWith('#tb_B2SubTable#') && flag.substr(-3) == '#0#') {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(1).find('input').val();//编码
					_row.v2 = tdArr.eq(2).find('input').val();//名称
					_row.v3 = tdArr.eq(3).find('input').val();//数量
					_list.push(_row);
				}
			}
			return _list;
		...
		getCurrentGridTable: function (tbid, id) {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_B2SubTable#' + id + '#') && tbid =='tb_B2SubTable') {
					_list.push({ ControlID: 'v1', ControlValue: tdArr.eq(1).find('input').val(), ControlType: '1' });//编码
					_list.push({ ControlID: 'v2', ControlValue: tdArr.eq(2).find('input').val(), ControlType: '1' });//名称
					_list.push({ ControlID: 'v3', ControlValue: tdArr.eq(3).find('input').val(), ControlType: '1' });//数量
				}
			}
			return _list;
		},
		...
		foreachGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_B2SubTable#')) {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(1).find('input').val();//编码
					_row.v2 = tdArr.eq(2).find('input').val();//名称
		...
		disabledGridTable: function (grid) {
			//控制子表工具栏按钮
			if (grid == 'tb_B2SubTable') {
				$('#del_tb_B2SubTable').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_B2SubTable').removeAttr('lay-event');
				$('#add_tb_B2SubTable').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_B2SubTable').removeAttr('lay-event');
				$('#edit_tb_B2SubTable').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_B2SubTable').removeAttr('lay-event');
				$('#add_tb_B2SubTable').attr('disabled', 'true');
				$('#edit_tb_B2SubTable').attr('disabled', 'true');
				$('#del_tb_B2SubTable').attr('disabled', 'true');
			}
			var trList = $(".layui-table").find("tr");  //获取table下的所有tr
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_B2SubTable#') && grid =='tb_B2SubTable') {
					tdArr.eq(1).find('input').attr('disabled', 'disabled');//编码
					tdArr.eq(1).find('button').unbind('click');//编码
					tdArr.eq(2).find('input').attr('disabled', 'disabled');//名称
					tdArr.eq(3).find('input').attr('disabled', 'disabled');//数量
	...
	//暴露接口
	exports('B2WriteBackTheGoal', api);
});