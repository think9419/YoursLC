...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
        ...
        disableGridColumnFormControl: function (grid, strDisable) {
			if (grid =='tb_B2WriteBackSubTable') {
				...
				if (strDisable.indexOf('[v2]') > -1) {
					$('#v2').attr('disabled', 'disabled');//数量
					$('#v2').removeAttr('lay-verify');//数量
					$('#v2').attr('class', 'layui-input layui-disabled')
					$('#select_v2').unbind('click');
				}
				if (strDisable.indexOf('[v3]') > -1) {
					$('#v3').attr('disabled', 'disabled');//备注
					$('#v3').removeAttr('lay-verify');//备注
		...
		getControlListFrmGridColumn: function (grid) {
			...
			if (grid =='tb_B2WriteBackSubTable') {
				...
				_list.push({ ControlID: 'v2', ControlValue: $('#v2').val(), ControlType: '1' });//数量
				_list.push({ ControlID: 'v3', ControlValue: $('#v3').val(), ControlType: '1' });//备注
			}
			return _list;
		},
		...
		setValueByList: function (lists, _tbid, _rowid) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//
                ...
                for (var i = 0; i < trList.length; i++) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_B2WriteBackSubTable') {
						for (var item in lists) {
		...
		getValueFromPopUp: function (_tbid, _indexid, _id, _v, _value) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//主表或者子表纵列
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if (flag.startsWith(_starts) && _tbid == 'tb_B2WriteBackSubTable') {
		...
		getFirstGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				...
				if (flag.startsWith('#tb_B2WriteBackSubTable#') && flag.substr(-3) == '#0#') {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(1).find('input').val();//编码
					_row.v2 = tdArr.eq(2).find('input').val();//数量
					_row.v3 = tdArr.eq(3).find('input').val();//备注
					_list.push(_row);
				}
			}
			return _list;
		...
		getCurrentGridTable: function (tbid, id) {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_B2WriteBackSubTable#' + id + '#') && tbid =='tb_B2WriteBackSubTable') {
					_list.push({ ControlID: 'v1', ControlValue: tdArr.eq(1).find('input').val(), ControlType: '1' });//编码
					_list.push({ ControlID: 'v2', ControlValue: tdArr.eq(2).find('input').val(), ControlType: '1' });//数量
					_list.push({ ControlID: 'v3', ControlValue: tdArr.eq(3).find('input').val(), ControlType: '1' });//备注
				}
			}
			return _list;
		},
		...
		foreachGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_B2WriteBackSubTable#')) {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(1).find('input').val();//编码
					_row.v2 = tdArr.eq(2).find('input').val();//数量
		...
		disabledGridTable: function (grid) {
			//控制子表工具栏按钮
			if (grid == 'tb_B2WriteBackSubTable') {
				$('#del_tb_B2WriteBackSubTable').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_B2WriteBackSubTable').removeAttr('lay-event');
				$('#add_tb_B2WriteBackSubTable').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_B2WriteBackSubTable').removeAttr('lay-event');
				$('#edit_tb_B2WriteBackSubTable').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_B2WriteBackSubTable').removeAttr('lay-event');
				$('#add_tb_B2WriteBackSubTable').attr('disabled', 'true');
				$('#edit_tb_B2WriteBackSubTable').attr('disabled', 'true');
				$('#del_tb_B2WriteBackSubTable').attr('disabled', 'true');
			}
			var trList = $(".layui-table").find("tr");  //获取table下的所有tr
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_B2WriteBackSubTable#') && grid =='tb_B2WriteBackSubTable') {
					tdArr.eq(1).find('input').attr('disabled', 'disabled');//编码
					tdArr.eq(1).find('button').unbind('click');//编码
					tdArr.eq(2).find('input').attr('disabled', 'disabled');//数量
					tdArr.eq(3).find('input').attr('disabled', 'disabled');//备注
