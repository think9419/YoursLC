...
layui.define(["layer"], function (exprots) {
    ...
    var exUtils = {
        ...
        ajax: function (url, type, params, load) {
            ...
            var loadIndex;
            $.ajax({
                url: exUtils.isFrontendBackendSeparate ? exUtils.baseUrl + url : url,
                type: type || "get",
                data: params || {},
                dataType: "json",
                beforeSend: function () {
                ...
                success: function (data) {
                    if (data.state == "success") {
                        ...
                        // 业务异常
                        layer.msg(data.message, { icon: 7, time: 20000, btn: ['关闭'] });
                        deferred.reject("exUtils.ajax warn: " + data.message);
                    }
                },
                complete: function () {
                    if (load) {
                ...
                error: function () {
                    ...
                    deferred.reject("exUtils.ajax error: 服务器错误");
                }
            });
        ...
        tableBatchCheckByKey: function (table, key) {
            var checkStatus = table.checkStatus("tableId");
            var rows = checkStatus.data.length;
            if (rows > 0) {
                var idsStr = "";
                for (var i = 0; i < checkStatus.data.length; i++) {
                    idsStr += checkStatus.data[i].key + ",";
                }
                return idsStr;
            } else {
                layer.msg("未选择有效数据", { offset: "t", anim: 6 });
