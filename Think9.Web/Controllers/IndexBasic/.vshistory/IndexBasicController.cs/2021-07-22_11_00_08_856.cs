...
namespace Think9.Web.Areas.SystemOrganize.Controllers
{
    ...
    {
        //zzz
        public IndexBasic IndexBasic = new IndexBasic();
        ...
        {
            var result = IndexBasic.GetListByFilter(model, pageInfo);
            return Json(result);
        }
        ...
        public ActionResult Add(RoleModel model)
        {
            ...
            if (IndexBasic.GetTotal(where, new { EnCode = model.EnCode}) > 0)
            {
            ...
            else
            {
                ...
                var result = IndexBasic.Insert(model) ? SuccessTip("添加成功") : ErrorTip("添加失败");
                return Json(result);
            }
        }
        ...
        {
            ViewBag.RoleTypeList = RoleTypeList;
            var model = IndexBasic.GetById(id);
            return View(model);
        }
        ...
        public ActionResult Edit(RoleModel model)
        {
            model.UpdateTime = DateTime.Now;
            model.UpdateUserId = Operator.UserId;
            var result = IndexBasic.UpdateById(model) ? SuccessTip("修改成功") : ErrorTip("修改失败");
            return Json(result);
        }
        ...
        public JsonResult Delete(int id)
        {
            ...
            {
                var result = IndexBasic.DeleteById(id) ? SuccessTip("删除成功") : ErrorTip("删除失败");
                return Json(result);
            }
        }
