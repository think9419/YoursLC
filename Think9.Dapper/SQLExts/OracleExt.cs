...
namespace DapperExtensions.OracleExt
{
    public static partial class OracleExt
    {
        ...
        //public static IEnumerable<T> GetPageList3333<T>(this IDbConnection conn, out long total, string files, string tableName, string where, string orderby, int pageIndex, int pageSize, object param = null, IDbTransaction transaction = null, int? commandTimeout = null)
        //{
        //    int selectedId = 1;
        ...
        //    var dynParams = new OracleDynamicParameters(param, refCursorNames);
        ...
        //    using (var reader = conn.QueryMultiple(sql, dynParams, transaction, commandTimeout))
        //    {
        ...
        public static IEnumerable<T> GetPageList<T>(this IDbConnection conn, out long total, string files, string tableName, string where, string orderby, int pageIndex, int pageSize, object param = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            dpar.AddDynamicParams(param);

            sql = "SELECT COUNT(1) FROM \"" + tableName + "\" " + where;
            object obj = conn.ExecuteScalar(sql, dpar);
            ...
            //var oracleParameter = new DynamicParameters(refCursorName, OracleDbType.RefCursor, ParameterDirection.Output);
            //oracleParameters.Add(oracleParameter);

            //string[] refCursorNames = { "rslt1", "rslt2" };
            //var dynParams = new OracleDynamicParameters(param, refCursorNames);
        ...
        public static DataTable GetTableAndViewList(this IDbConnection conn, string dbName)
        {
            ...
            foreach (DataRow dr in GetDataTable(conn, "SELECT TABLE_NAME FROM ALL_TABLES where owner='" + dbName + "'").Rows)
            {
            ...
            foreach (DataRow dr in GetDataTable(conn, "SELECT VIEW_NAME FROM ALL_VIEWS where owner='" + dbName + "'").Rows)
            {
                DataRow row = dt.NewRow();
                row["table_name"] = dr["VIEW_NAME"].ToString();
                dt.Rows.Add(row);
            }
        ...
        {
            return GetDataTable(conn, "SELECT table_name , column_name as COLUMN_NAME, data_type as DATA_type, data_length FROM all_tab_columns WHERE table_name = upper('" + tbID + "')");
        }
