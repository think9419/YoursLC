...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
		...
		disabledControls: function (prcno, _fields, _hiddens) {
			...
			//数量
			if (_fields != '#all#' && _fields.indexOf(',inQuantity,') == -1) {
				$('#inQuantity').attr('disabled','disabled');//禁用
				$('#inQuantity').attr('class','layui-input layui-disabled');
				$('#inQuantity').removeAttr('lay-verify');//去除校验
				$('#select_inQuantity').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inQuantity;') != -1) {
					$('#inQuantity').attr('placeholder','******');//保密
					$('#inQuantity').val('');
				}
			}
			//填报人
		...
		getControlValueList: function () {
			...
			_list.push({ ControlID: 'inQuantity', ControlValue: $('#inQuantity').val(), ControlType: '1' });//数量
		...
		getControlValueListFrmParent: function () {
			...
			_list.push({ ControlID: 'inQuantity', ControlValue: $(window.parent.document).find('#inQuantity').val(), ControlType: '1' });//数量
	...
	//暴露接口
	exports('A1WriteBackTheTarget', api);
});