...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
		...
		disabledControls: function (prcno, _fields, _hiddens) {
			...
			//字符01
			if (_fields != '#all#' && _fields.indexOf(',inZF01,') == -1) {
				$('#inZF01').attr('disabled','disabled');//禁用
				$('#inZF01').attr('class','layui-input layui-disabled');
				$('#inZF01').removeAttr('lay-verify');//去除校验
				$('#select_inZF01').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inZF01;') != -1) {
					$('#inZF01').attr('placeholder','******');//保密
					$('#inZF01').val('');
				}
			}
			//字符02
			if (_fields != '#all#' && _fields.indexOf(',inZF02,') == -1) {
				$('#inZF02').attr('disabled','disabled');//禁用
				$('#inZF02').attr('class','layui-input layui-disabled');
				$('#inZF02').removeAttr('lay-verify');//去除校验
				$('#select_inZF02').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inZF02;') != -1) {
					$('#inZF02').attr('placeholder','******');//保密
					$('#inZF02').val('');
				}
			}
			//日期01
			if (_fields != '#all#' && _fields.indexOf(',inRQ01,') == -1) {
				$('#inRQ01').attr('disabled','disabled');//禁用
				$('#inRQ01').attr('class','layui-input layui-disabled');
				$('#inRQ01').removeAttr('lay-verify');//去除校验
				$('#select_inRQ01').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inRQ01;') != -1) {
					$('#inRQ01').attr('placeholder','******');//保密
					$('#inRQ01').val('');
				}
			}
			//日期02
			if (_fields != '#all#' && _fields.indexOf(',inRQ02,') == -1) {
				$('#inRQ02').attr('disabled','disabled');//禁用
				$('#inRQ02').attr('class','layui-input layui-disabled');
				$('#inRQ02').removeAttr('lay-verify');//去除校验
				$('#select_inRQ02').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inRQ02;') != -1) {
					$('#inRQ02').attr('placeholder','******');//保密
					$('#inRQ02').val('');
				}
			}
			//小数01
		...
		getControlValueList: function () {
			...
			_list.push({ ControlID: 'inZF01', ControlValue: $('#inZF01').val(), ControlType: '1' });//字符01
			_list.push({ ControlID: 'inZF02', ControlValue: $('#inZF02').val(), ControlType: '1' });//字符02
			_list.push({ ControlID: 'inRQ01', ControlValue: $('#inRQ01').val(), ControlType: '1' });//日期01
			_list.push({ ControlID: 'inRQ02', ControlValue: $('#inRQ02').val(), ControlType: '1' });//日期02
		...
		getControlValueListFrmParent: function () {
		    var _list = [];
			_list.push({ ControlID: 'listid', ControlValue: $('#_listid').val(), ControlType: '1' });
			_list.push({ ControlID: 'inZF01', ControlValue: $(window.parent.document).find('#inZF01').val(), ControlType: '1' });//字符01
			_list.push({ ControlID: 'inZF02', ControlValue: $(window.parent.document).find('#inZF02').val(), ControlType: '1' });//字符02
			_list.push({ ControlID: 'inRQ01', ControlValue: $(window.parent.document).find('#inRQ01').val(), ControlType: '1' });//日期01
			_list.push({ ControlID: 'inRQ02', ControlValue: $(window.parent.document).find('#inRQ02').val(), ControlType: '1' });//日期02
	...
	//暴露接口
	exports('CustomVerification', api);
});