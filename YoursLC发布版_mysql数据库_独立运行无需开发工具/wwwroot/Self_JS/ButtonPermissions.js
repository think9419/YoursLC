	...
	var api = {
		...
		disabledControls: function (prcno, _fields, _hiddens) {
			...
			if (_fields != '#all#' && _fields.indexOf(',inAuditing,') == -1) {
				$("input[name='inAuditing']").attr('disabled','disabled');//禁用
				if (_hiddens.indexOf(';inAuditing;') != -1) {
					$('#inAuditing').val('');//保密
					$("input[name='inAuditing']").prop('checked', false);
				}
			}
			//省
        ...
        disableListBtn: function (strDisable) {
			...
			if (strDisable == 'all' || strDisable.indexOf('[btn_Locking]') > -1) {
				$('#btn_Locking').attr('class', 'layui-btn layui-btn-disabled layui-btn-sm');//禁用锁定
				$('#btn_Locking').removeAttr('lay-event');
			}
			if (strDisable == 'all' || strDisable.indexOf('[btn_UnLock]') > -1) {
				$('#btn_UnLock').attr('class', 'layui-btn layui-btn-disabled layui-btn-sm');//禁用解锁
				$('#btn_UnLock').removeAttr('lay-event');
			}
		},
		...
		getControlValueList: function () {
			...
			_list.push({ ControlID: 'inAuditing', ControlValue: $("input[name='inAuditing']:checked").val(), ControlType: '4' });//审核
		...
		getControlValueListFrmParent: function () {
		    var _list = [];
			_list.push({ ControlID: 'listid', ControlValue: $('#_listid').val(), ControlType: '1' });
			_list.push({ ControlID: 'inAuditing', ControlValue: $(window.parent.document).find("input[name='inAuditing']:checked").val(), ControlType: '4' });//审核
