...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
		...
		lockForEditing: function (_type) {
			if (_type == 'edit') {
				$('#inLockDuringEditing').attr('disabled','disabled');//编辑时锁定
				$('#inLockDuringEditing').attr('class','layui-input layui-disabled');//编辑时锁定
				$('#select_inLockDuringEditing').unbind('click');//编辑时锁定
			}
		},
		...
		disabledControls: function (prcno, _fields, _hiddens) {
			...
			//编辑时锁定
			if (_fields != '#all#' && _fields.indexOf(',inLockDuringEditing,') == -1) {
				$('#inLockDuringEditing').attr('disabled','disabled');//禁用
				$('#inLockDuringEditing').attr('class','layui-input layui-disabled');
				$('#inLockDuringEditing').removeAttr('lay-verify');//去除校验
				$('#select_inLockDuringEditing').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inLockDuringEditing;') != -1) {
					$('#inLockDuringEditing').attr('placeholder','******');//保密
					$('#inLockDuringEditing').val('');
				}
			}
			//锁定
			if (_fields != '#all#' && _fields.indexOf(',inLocking,') == -1) {
				$('#inLocking').attr('disabled','disabled');//禁用
				$('#inLocking').attr('class','layui-input layui-disabled');
				$('#inLocking').removeAttr('lay-verify');//去除校验
				$('#select_inLocking').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inLocking;') != -1) {
					$('#inLocking').attr('placeholder','******');//保密
					$('#inLocking').val('');
				}
			}
			//不锁定
			if (_fields != '#all#' && _fields.indexOf(',inNotLocked,') == -1) {
				$('#inNotLocked').attr('disabled','disabled');//禁用
				$('#inNotLocked').attr('class','layui-input layui-disabled');
				$('#inNotLocked').removeAttr('lay-verify');//去除校验
				$('#select_inNotLocked').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inNotLocked;') != -1) {
					$('#inNotLocked').attr('placeholder','******');//保密
					$('#inNotLocked').val('');
				}
			}
		},
		...
		getControlValueList: function () {
			...
			_list.push({ ControlID: 'inLockDuringEditing', ControlValue: $('#inLockDuringEditing').val(), ControlType: '1' });//编辑时锁定
			_list.push({ ControlID: 'inLocking', ControlValue: $('#inLocking').val(), ControlType: '1' });//锁定
			_list.push({ ControlID: 'inNotLocked', ControlValue: $('#inNotLocked').val(), ControlType: '1' });//不锁定
			return _list;
		},
		...
		getControlValueListFrmParent: function () {
		    var _list = [];
			_list.push({ ControlID: 'listid', ControlValue: $('#_listid').val(), ControlType: '1' });
			_list.push({ ControlID: 'inLockDuringEditing', ControlValue: $(window.parent.document).find('#inLockDuringEditing').val(), ControlType: '1' });//编辑时锁定
			_list.push({ ControlID: 'inLocking', ControlValue: $(window.parent.document).find('#inLocking').val(), ControlType: '1' });//锁定
			_list.push({ ControlID: 'inNotLocked', ControlValue: $(window.parent.document).find('#inNotLocked').val(), ControlType: '1' });//不锁定
			return _list;
		},
		...
		getControlListFrmGridColumn: function (grid) {
			...
			_list.push({ ControlID: 'listid', ControlValue: $('#_listid').val(), ControlType: '1' });
			if (grid =='tb_Subtable04') {
				_list.push({ ControlID: 'v1', ControlValue: $('#v1').val(), ControlType: '1' });//锁定
				_list.push({ ControlID: 'v2', ControlValue: $('#v2').val(), ControlType: '1' });//不锁定
				_list.push({ ControlID: 'v3', ControlValue: $('#v3').val(), ControlType: '1' });//编辑时锁定
				_list.push({ ControlID: 'v4', ControlValue: $('#v4').val(), ControlType: '1' });//按行锁定
			}
			return _list;
		},
		...
		setValueByList: function (lists, _tbid, _rowid) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//
                ...
                for (var i = 0; i < trList.length; i++) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_Subtable04') {
						for (var item in lists) {
		...
		getValueFromPopUp: function (_tbid, _indexid, _id, _v, _value) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//主表或者子表纵列
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if (flag.startsWith(_starts) && _tbid == 'tb_Subtable04') {
		...
		getFirstGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				...
				if (flag.startsWith('#tb_Subtable04#') && flag.substr(-3) == '#0#') {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(1).find('input').val();//锁定
					_row.v2 = tdArr.eq(2).find('input').val();//不锁定
					_row.v3 = tdArr.eq(3).find('input').val();//编辑时锁定
					_row.v4 = tdArr.eq(4).find('input').val();//按行锁定
					_list.push(_row);
				}
			}
			return _list;
		...
		getCurrentGridTable: function (tbid, id) {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable04#' + id + '#') && tbid =='tb_Subtable04') {
					_list.push({ ControlID: 'v1', ControlValue: tdArr.eq(1).find('input').val(), ControlType: '1' });//锁定
					_list.push({ ControlID: 'v2', ControlValue: tdArr.eq(2).find('input').val(), ControlType: '1' });//不锁定
					_list.push({ ControlID: 'v3', ControlValue: tdArr.eq(3).find('input').val(), ControlType: '1' });//编辑时锁定
					_list.push({ ControlID: 'v4', ControlValue: tdArr.eq(4).find('input').val(), ControlType: '1' });//按行锁定
				}
			}
			return _list;
		},
		...
		foreachGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable04#')) {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(1).find('input').val();//锁定
					_row.v2 = tdArr.eq(2).find('input').val();//不锁定
		...
		disabledGridTable: function (grid) {
			//控制子表工具栏按钮
			if (grid == 'tb_Subtable04') {
				$('#del_tb_Subtable04').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_Subtable04').removeAttr('lay-event');
				$('#add_tb_Subtable04').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_Subtable04').removeAttr('lay-event');
				$('#edit_tb_Subtable04').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_Subtable04').removeAttr('lay-event');
				$('#add_tb_Subtable04').attr('disabled', 'true');
				$('#edit_tb_Subtable04').attr('disabled', 'true');
				$('#del_tb_Subtable04').attr('disabled', 'true');
			}
			var trList = $(".layui-table").find("tr");  //获取table下的所有tr
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable04#') && grid =='tb_Subtable04') {
					tdArr.eq(1).find('input').attr('disabled', 'disabled');//锁定
					tdArr.eq(2).find('input').attr('disabled', 'disabled');//不锁定
					tdArr.eq(3).find('input').attr('disabled', 'disabled');//编辑时锁定
					tdArr.eq(4).find('input').attr('disabled', 'disabled');//按行锁定
