...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
		...
		disabledControls: function (prcno, _fields, _hiddens) {
			...
			//ListID
			if (_fields != '#all#' && _fields.indexOf(',inListID,') == -1) {
				$('#inListID').attr('disabled','disabled');//禁用
				$('#inListID').attr('class','layui-input layui-disabled');
				$('#inListID').removeAttr('lay-verify');//去除校验
				$('#select_inListID').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inListID;') != -1) {
					$('#inListID').attr('placeholder','******');//保密
					$('#inListID').val('');
				}
			}
			//ID
			if (_fields != '#all#' && _fields.indexOf(',inID,') == -1) {
				$('#inID').attr('disabled','disabled');//禁用
				$('#inID').attr('class','layui-input layui-disabled');
				$('#inID').removeAttr('lay-verify');//去除校验
				$('#select_inID').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inID;') != -1) {
					$('#inID').attr('placeholder','******');//保密
					$('#inID').val('');
				}
			}
			//编码
		...
		getControlValueList: function () {
			...
			_list.push({ ControlID: 'inListID', ControlValue: $('#inListID').val(), ControlType: '1' });//ListID
			_list.push({ ControlID: 'inID', ControlValue: $('#inID').val(), ControlType: '1' });//ID
			...
			_list.push({ ControlID: 'inRemarks', ControlValue: $('#inRemarks').val(), ControlType: '2' });//备注
			return _list;
		},
		...
		getControlValueListFrmParent: function () {
		    var _list = [];
			_list.push({ ControlID: 'listid', ControlValue: $('#_listid').val(), ControlType: '1' });
			_list.push({ ControlID: 'inListID', ControlValue: $(window.parent.document).find('#inListID').val(), ControlType: '1' });//ListID
			_list.push({ ControlID: 'inID', ControlValue: $(window.parent.document).find('#inID').val(), ControlType: '1' });//ID
			...
			_list.push({ ControlID: 'inRemarks', ControlValue: $(window.parent.document).find('#inRemarks').val(), ControlType: '2' });//备注
			return _list;
		},
