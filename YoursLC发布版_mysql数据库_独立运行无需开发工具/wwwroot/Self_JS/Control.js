...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
		...
		disabledControls: function (prcno, _fields, _hiddens) {
			...
			//图片
			if (_fields != '#all#' && _fields.indexOf(',inPicture,') == -1) {
				$('#inPicture').removeAttr('lay-verify');//禁用
				$('#inPicture').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inPicture;') != -1) {
					$('#inPicture_Exa').val('');//保密
			...
			//下拉树
			if (_fields != '#all#' && _fields.indexOf(',inDropdownTree,') == -1) {
				$('#inDropdownTree').attr('disabled','disabled');//禁用
				$('#inDropdownTree').removeAttr('lay-verify');//去除校验
				if (_hiddens.indexOf(';inDropdownTree;') != -1) {
					$('#inDropdownTree').val('');//保密
				}
			}
			//弹出选择
			...
			//多选
			if (_fields != '#all#' && _fields.indexOf(',inMultipleChoice,') == -1) {
				$("input[name='inMultipleChoice']").attr('disabled','disabled');//禁用
				if (_hiddens.indexOf(';inMultipleChoice;') != -1) {
					$('#inMultipleChoice_Exa').val('');//保密
					$("input[name='inMultipleChoice']").prop('checked', false);//
				}
			}
			//单选
			if (_fields != '#all#' && _fields.indexOf(',inSingleChoice,') == -1) {
				$("input[name='inSingleChoice']").attr('disabled','disabled');//禁用
				if (_hiddens.indexOf(';inSingleChoice;') != -1) {
					$('#inSingleChoice').val('');//保密
					$("input[name='inSingleChoice']").prop('checked', false);
				}
			}
			//下拉选择
			...
			//附件
			if (_fields != '#all#' && _fields.indexOf(',inAttachment,') == -1) {
				$('#inAttachment').attr('disabled','disabled');//禁用
				$('#inAttachment').attr('class','layui-input layui-disabled');
				$('#inAttachment').removeAttr('lay-verify');//去除校验
				$('#select_inAttachment').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inAttachment;') != -1) {
					$('#inAttachment').attr('placeholder','******');//保密
					$('#inAttachment').val('');
				}
			}
		},
		...
		getControlValueList: function () {
			...
			_list.push({ ControlID: 'inPicture', ControlValue: $('#inPicture_Exa').val(), ControlType: '5' });//图片
			_list.push({ ControlID: 'inDate', ControlValue: $('#inDate').val(), ControlType: '1' });//日期
			_list.push({ ControlID: 'inDropdownTree', ControlValue: $('#inDropdownTree').val(), ControlType: '2' });//下拉树
			_list.push({ ControlID: 'inPopUpSelection', ControlValue: $('#inPopUpSelection').val(), ControlType: '1' });//弹出选择
			_list.push({ ControlID: 'inMultipleChoice', ControlValue: $('#inMultipleChoice_Exa').val(), ControlType: '3' });//多选
			_list.push({ ControlID: 'inSingleChoice', ControlValue: $("input[name='inSingleChoice']:checked").val(), ControlType: '4' });//单选
			_list.push({ ControlID: 'inDropdownSelection', ControlValue: $('#inDropdownSelection').val(), ControlType: '2' });//下拉选择
			_list.push({ ControlID: 'inAttachment', ControlValue: $('#inAttachment').val(), ControlType: '1' });//附件
			return _list;
		},
		...
		getControlValueListFrmParent: function () {
		    var _list = [];
			_list.push({ ControlID: 'listid', ControlValue: $('#_listid').val(), ControlType: '1' });
			_list.push({ ControlID: 'inPicture', ControlValue: $(window.parent.document).find('#inPicture_Exa').val(), ControlType: '5' });//图片
			_list.push({ ControlID: 'inDate', ControlValue: $(window.parent.document).find('#inDate').val(), ControlType: '1' });//日期
			_list.push({ ControlID: 'inDropdownTree', ControlValue: $(window.parent.document).find('#inDropdownTree').val(), ControlType: '2' });//下拉树
			_list.push({ ControlID: 'inPopUpSelection', ControlValue: $(window.parent.document).find('#inPopUpSelection').val(), ControlType: '1' });//弹出选择
			_list.push({ ControlID: 'inMultipleChoice', ControlValue: $(window.parent.document).find('#inMultipleChoice_Exa').val(), ControlType: '3' });//多选
			_list.push({ ControlID: 'inSingleChoice', ControlValue: $(window.parent.document).find("input[name='inSingleChoice']:checked").val(), ControlType: '4' });//单选
			...
			_list.push({ ControlID: 'inAttachment', ControlValue: $(window.parent.document).find('#inAttachment').val(), ControlType: '1' });//附件
			return _list;
		},
		...
		getControlListFrmGridColumn: function (grid) {
			...
			_list.push({ ControlID: 'listid', ControlValue: $('#_listid').val(), ControlType: '1' });
			if (grid =='tb_Subtable03') {
				_list.push({ ControlID: 'v1', ControlValue: $('#v1').val(), ControlType: '1' });//日期
				_list.push({ ControlID: 'v2', ControlValue: $('#v2').val(), ControlType: '1' });//文本框
				_list.push({ ControlID: 'v3', ControlValue: $('#v3').val(), ControlType: '1' });//弹出选择
				_list.push({ ControlID: 'v4', ControlValue: $('#v4').val(), ControlType: '2' });//下拉选择
				_list.push({ ControlID: 'v5', ControlValue: $('#v5_Exa').val(), ControlType: '5' });//图片
				_list.push({ ControlID: 'v6', ControlValue: $('#v6').val(), ControlType: '1' });//附件
			}
			return _list;
		},
		...
		setValueByList: function (lists, _tbid, _rowid) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//
                ...
                for (var i = 0; i < trList.length; i++) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_Subtable03') {
						for (var item in lists) {
							...
							if (_list == null) {//读取单值
								if (lists[item].ControlType == "2") {
									tdArr.eq(parseInt(_id) + 1).find('select').val(_value);//下拉选择 错位（有复选框）
								}
								else {
								...
								if (lists[item].ControlType == "2") {
									tdArr.eq(parseInt(_id) + 1).find('select').empty();//错位（有复选框）
									$.each(_list, function (i, item) {
		...
		getValueFromPopUp: function (_tbid, _indexid, _id, _v, _value) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//主表或者子表纵列
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if (flag.startsWith(_starts) && _tbid == 'tb_Subtable03') {
		...
		getFirstGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				...
				if (flag.startsWith('#tb_Subtable03#') && flag.substr(-3) == '#0#') {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//日期
					_row.v2 = tdArr.eq(3).find('input').val();//文本框
					_row.v3 = tdArr.eq(4).find('input').val();//弹出选择
					_row.v4 = tdArr.eq(5).find('select').val();//下拉选择
					_row.v5 = tdArr.eq(6).find('input').val();//图片
					_row.v6 = tdArr.eq(7).find('input').val();//附件
					_list.push(_row);
				}
			}
			return _list;
		...
		getCurrentGridTable: function (tbid, id) {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable03#' + id + '#') && tbid =='tb_Subtable03') {
					_list.push({ ControlID: 'v1', ControlValue: tdArr.eq(2).find('input').val(), ControlType: '1' });//日期
					_list.push({ ControlID: 'v2', ControlValue: tdArr.eq(3).find('input').val(), ControlType: '1' });//文本框
					_list.push({ ControlID: 'v3', ControlValue: tdArr.eq(4).find('input').val(), ControlType: '1' });//弹出选择
					_list.push({ ControlID: 'v4', ControlValue: tdArr.eq(5).find('select').val(), ControlType: '2' });//下拉选择
					_list.push({ ControlID: 'v5', ControlValue: tdArr.eq(6).find('input').val(), ControlType: '5' });//图片
					_list.push({ ControlID: 'v6', ControlValue: tdArr.eq(7).find('input').val(), ControlType: '1' });//附件
				}
			}
			return _list;
		},
		...
		foreachGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable03#')) {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//日期
					_row.v2 = tdArr.eq(3).find('input').val();//文本框
		...
		disabledGridTable: function (grid) {
			//控制子表工具栏按钮
			if (grid == 'tb_Subtable03') {
				$('#del_tb_Subtable03').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_Subtable03').removeAttr('lay-event');
				$('#add_tb_Subtable03').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_Subtable03').removeAttr('lay-event');
				$('#edit_tb_Subtable03').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_Subtable03').removeAttr('lay-event');
				$('#add_tb_Subtable03').attr('disabled', 'true');
				$('#edit_tb_Subtable03').attr('disabled', 'true');
				$('#del_tb_Subtable03').attr('disabled', 'true');
			}
			var trList = $(".layui-table").find("tr");  //获取table下的所有tr
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable03#') && grid =='tb_Subtable03') {
					tdArr.eq(2).find('input').attr('disabled', 'disabled');//日期
					tdArr.eq(3).find('input').attr('disabled', 'disabled');//文本框
					tdArr.eq(4).find('input').attr('disabled', 'disabled');//弹出选择
					tdArr.eq(4).find('button').unbind('click');//弹出选择
					tdArr.eq(5).find('select').attr('disabled', 'disabled');//下拉选择
					tdArr.eq(6).find('input').attr('disabled', 'disabled');//图片
					tdArr.eq(7).find('input').attr('disabled', 'disabled');//附件
