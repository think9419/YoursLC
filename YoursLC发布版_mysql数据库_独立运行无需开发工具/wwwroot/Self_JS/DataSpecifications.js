...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
        ...
        disableGridColumnFormControl: function (grid, strDisable) {
			if (grid =='tb_Subtable09') {
				if (strDisable.indexOf('[v1]') > -1) {
					$('#v1').attr('disabled', 'disabled');//下拉选择
					$('#v1').removeAttr('lay-verify');//下拉选择
				}
				if (strDisable.indexOf('[v2]') > -1) {
					$('#v2').attr('disabled', 'disabled');//弹出选择
					$('#v2').removeAttr('lay-verify');//弹出选择
					$('#v2').attr('class', 'layui-input layui-disabled')
					$('#select_v2').unbind('click');
				}
				if (strDisable.indexOf('[v3]') > -1) {
					$('#v3').attr('disabled', 'disabled');//下拉树
					$('#v3').removeAttr('lay-verify');//下拉树
				}
			}
        },
		...
		getControlListFrmGridColumn: function (grid) {
			...
			_list.push({ ControlID: 'listid', ControlValue: $('#_listid').val(), ControlType: '1' });
			if (grid =='tb_Subtable09') {
				_list.push({ ControlID: 'v1', ControlValue: $('#v1').val(), ControlType: '2' });//下拉选择
				_list.push({ ControlID: 'v2', ControlValue: $('#v2').val(), ControlType: '1' });//弹出选择
				_list.push({ ControlID: 'v3', ControlValue: $('#v3').val(), ControlType: '2' });//下拉树
			}
			return _list;
		},
		...
		setValueByList: function (lists, _tbid, _rowid) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//
                ...
                for (var i = 0; i < trList.length; i++) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_Subtable09') {
						for (var item in lists) {
		...
		getValueFromPopUp: function (_tbid, _indexid, _id, _v, _value) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//主表或者子表纵列
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if (flag.startsWith(_starts) && _tbid == 'tb_Subtable09') {
		...
		getFirstGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				...
				if (flag.startsWith('#tb_Subtable09#') && flag.substr(-3) == '#0#') {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(1).find('select').val();//下拉选择
					_row.v2 = tdArr.eq(2).find('input').val();//弹出选择
					_row.v3 = tdArr.eq(3).find('select').val();//下拉树
					_list.push(_row);
				}
			}
			return _list;
		...
		getCurrentGridTable: function (tbid, id) {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable09#' + id + '#') && tbid =='tb_Subtable09') {
					_list.push({ ControlID: 'v1', ControlValue: tdArr.eq(1).find('select').val(), ControlType: '2' });//下拉选择
					_list.push({ ControlID: 'v2', ControlValue: tdArr.eq(2).find('input').val(), ControlType: '1' });//弹出选择
					_list.push({ ControlID: 'v3', ControlValue: tdArr.eq(3).find('select').val(), ControlType: '2' });//下拉树
				}
			}
			return _list;
		},
		...
		foreachGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable09#')) {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(1).find('select').val();//下拉选择
					_row.v2 = tdArr.eq(2).find('input').val();//弹出选择
		...
		disabledGridTable: function (grid) {
			//控制子表工具栏按钮
			if (grid == 'tb_Subtable09') {
				$('#del_tb_Subtable09').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_Subtable09').removeAttr('lay-event');
				$('#add_tb_Subtable09').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_Subtable09').removeAttr('lay-event');
				$('#edit_tb_Subtable09').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_Subtable09').removeAttr('lay-event');
				$('#add_tb_Subtable09').attr('disabled', 'true');
				$('#edit_tb_Subtable09').attr('disabled', 'true');
				$('#del_tb_Subtable09').attr('disabled', 'true');
			}
			var trList = $(".layui-table").find("tr");  //获取table下的所有tr
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable09#') && grid =='tb_Subtable09') {
					tdArr.eq(1).find('select').attr('disabled', 'disabled');//下拉选择
					tdArr.eq(2).find('input').attr('disabled', 'disabled');//弹出选择
					tdArr.eq(2).find('button').unbind('click');//弹出选择
					tdArr.eq(3).find('select').attr('disabled', 'disabled');//下拉树
					tdArr.eq(4).find('button').attr('disabled', 'disabled');//右侧添加|删除按钮
					tdArr.eq(4).find('button').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
					tdArr.eq(4).find('button').removeAttr('lay-event');
				}
			}
		},
	...
	//暴露接口
	exports('DataSpecifications', api);
});