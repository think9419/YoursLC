...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
		...
		disabledControls: function (prcno, _fields, _hiddens) {
			...
			//市县
			if (_fields != '#all#' && _fields.indexOf(',inCityAndCounty,') == -1) {
				$('#inCityAndCounty').attr('disabled','disabled');//禁用
				$('#inCityAndCounty').removeAttr('lay-verify');//去除校验
				if (_hiddens.indexOf(';inCityAndCounty;') != -1) {
					$('#inCityAndCounty').val('');//保密
				}
			}
			//乡镇
			if (_fields != '#all#' && _fields.indexOf(',inTownship,') == -1) {
				$('#inTownship').attr('disabled','disabled');//禁用
				$('#inTownship').removeAttr('lay-verify');//去除校验
				if (_hiddens.indexOf(';inTownship;') != -1) {
					$('#inTownship').val('');//保密
				}
			}
			//备注
        ...
        disableGridColumnFormControl: function (grid, strDisable) {
			if (grid =='tb_Subtable07') {
				if (strDisable.indexOf('[v1]') > -1) {
					$('#v1').attr('disabled', 'disabled');//省1
					$('#v1').removeAttr('lay-verify');//省1
					$('#v1').attr('class', 'layui-input layui-disabled')
					$('#select_v1').unbind('click');
				}
				if (strDisable.indexOf('[v2]') > -1) {
					$('#v2').attr('disabled', 'disabled');//省2
					$('#v2').removeAttr('lay-verify');//省2
				}
				if (strDisable.indexOf('[v3]') > -1) {
					$('#v3').attr('disabled', 'disabled');//县市
					$('#v3').removeAttr('lay-verify');//县市
				}
				if (strDisable.indexOf('[v4]') > -1) {
					$('#v4').attr('disabled', 'disabled');//乡镇
					$('#v4').removeAttr('lay-verify');//乡镇
				}
		...
		getControlValueList: function () {
			...
			_list.push({ ControlID: 'inCityAndCounty', ControlValue: $('#inCityAndCounty').val(), ControlType: '2' });//市县
			_list.push({ ControlID: 'inTownship', ControlValue: $('#inTownship').val(), ControlType: '2' });//乡镇
		...
		getControlValueListFrmParent: function () {
			...
			_list.push({ ControlID: 'inCityAndCounty', ControlValue: $(window.parent.document).find('#inCityAndCounty').val(), ControlType: '2' });//市县
			_list.push({ ControlID: 'inTownship', ControlValue: $(window.parent.document).find('#inTownship').val(), ControlType: '2' });//乡镇
		...
		getControlListFrmGridColumn: function (grid) {
			...
			_list.push({ ControlID: 'listid', ControlValue: $('#_listid').val(), ControlType: '1' });
			if (grid =='tb_Subtable07') {
				_list.push({ ControlID: 'v1', ControlValue: $('#v1').val(), ControlType: '1' });//省1
				_list.push({ ControlID: 'v2', ControlValue: $('#v2').val(), ControlType: '2' });//省2
				_list.push({ ControlID: 'v3', ControlValue: $('#v3').val(), ControlType: '2' });//县市
				_list.push({ ControlID: 'v4', ControlValue: $('#v4').val(), ControlType: '2' });//乡镇
		...
		setValueByList: function (lists, _tbid, _rowid) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//
                ...
                for (var i = 0; i < trList.length; i++) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_Subtable07') {
						for (var item in lists) {
		...
		getValueFromPopUp: function (_tbid, _indexid, _id, _v, _value) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//主表或者子表纵列
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if (flag.startsWith(_starts) && _tbid == 'tb_Subtable07') {
		...
		getFirstGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				...
				if (flag.startsWith('#tb_Subtable07#') && flag.substr(-3) == '#0#') {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//省1
					_row.v2 = tdArr.eq(3).find('select').val();//省2
					_row.v3 = tdArr.eq(4).find('select').val();//县市
					_row.v4 = tdArr.eq(5).find('select').val();//乡镇
		...
		getCurrentGridTable: function (tbid, id) {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable07#' + id + '#') && tbid =='tb_Subtable07') {
					_list.push({ ControlID: 'v1', ControlValue: tdArr.eq(2).find('input').val(), ControlType: '1' });//省1
					_list.push({ ControlID: 'v2', ControlValue: tdArr.eq(3).find('select').val(), ControlType: '2' });//省2
					_list.push({ ControlID: 'v3', ControlValue: tdArr.eq(4).find('select').val(), ControlType: '2' });//县市
					_list.push({ ControlID: 'v4', ControlValue: tdArr.eq(5).find('select').val(), ControlType: '2' });//乡镇
		...
		foreachGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable07#')) {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//省1
					_row.v2 = tdArr.eq(3).find('select').val();//省2
		...
		disabledGridTable: function (grid) {
			//控制子表工具栏按钮
			if (grid == 'tb_Subtable07') {
				$('#del_tb_Subtable07').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_Subtable07').removeAttr('lay-event');
				$('#add_tb_Subtable07').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_Subtable07').removeAttr('lay-event');
				$('#edit_tb_Subtable07').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_Subtable07').removeAttr('lay-event');
				$('#add_tb_Subtable07').attr('disabled', 'true');
				$('#edit_tb_Subtable07').attr('disabled', 'true');
				$('#del_tb_Subtable07').attr('disabled', 'true');
			}
			var trList = $(".layui-table").find("tr");  //获取table下的所有tr
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_Subtable07#') && grid =='tb_Subtable07') {
					tdArr.eq(2).find('input').attr('disabled', 'disabled');//省1
					tdArr.eq(2).find('button').unbind('click');//省1
					tdArr.eq(3).find('select').attr('disabled', 'disabled');//省2
					tdArr.eq(4).find('select').attr('disabled', 'disabled');//县市
					tdArr.eq(5).find('select').attr('disabled', 'disabled');//乡镇
