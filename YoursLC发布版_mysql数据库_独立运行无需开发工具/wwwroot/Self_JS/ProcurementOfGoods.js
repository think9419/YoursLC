...
layui.define(['jquery', 'table'], function (exports) {
	...
	var api = {
		...
		disabledControls: function (prcno, _fields, _hiddens) {
			...
			//供应商
			if (_fields != '#all#' && _fields.indexOf(',inSupplier,') == -1) {
				$('#inSupplier').attr('disabled','disabled');//禁用
				$('#inSupplier').removeAttr('lay-verify');//去除校验
				if (_hiddens.indexOf(';inSupplier;') != -1) {
					$('#inSupplier').val('');//保密
				}
			}
			//经办人
			...
			//采购日期
			if (_fields != '#all#' && _fields.indexOf(',inPurchaseDate,') == -1) {
				$('#inPurchaseDate').attr('disabled','disabled');//禁用
				$('#inPurchaseDate').attr('class','layui-input layui-disabled');
				$('#inPurchaseDate').removeAttr('lay-verify');//去除校验
				$('#select_inPurchaseDate').unbind('click');//解除点击选择
				if (_hiddens.indexOf(';inPurchaseDate;') != -1) {
					$('#inPurchaseDate').attr('placeholder','******');//保密
					$('#inPurchaseDate').val('');
				}
			}
			//备注
        ...
        disableListBtn: function (strDisable) {
			...
			if (strDisable == 'all' || strDisable.indexOf('[btn_ReviewFailed]') > -1) {
				$('#btn_ReviewFailed').attr('class', 'layui-btn layui-btn-disabled layui-btn-sm');//禁用审核不通过
				$('#btn_ReviewFailed').removeAttr('lay-event');
			}
		},
		...
		getControlValueListFrmParent: function () {
			...
			_list.push({ ControlID: 'inSupplier', ControlValue: $(window.parent.document).find('#inSupplier').val(), ControlType: '2' });//供应商
			...
			_list.push({ ControlID: 'inPurchaseDate', ControlValue: $(window.parent.document).find('#inPurchaseDate').val(), ControlType: '1' });//采购日期
		...
		setValueByList: function (lists, _tbid, _rowid) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//
                ...
                for (var i = 0; i < trList.length; i++) {
					...
					if (tdArr.eq(0).text().startsWith(_starts) && _tbid == 'tb_ProcurementDetails') {
						for (var item in lists) {
		...
		getValueFromPopUp: function (_tbid, _indexid, _id, _v, _value) {
			...
			if (_tbid == '_main' || _tbid == '_gridColumn') {//主表或者子表纵列
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if (flag.startsWith(_starts) && _tbid == 'tb_ProcurementDetails') {
		...
		getFirstGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				...
				if (flag.startsWith('#tb_ProcurementDetails#') && flag.substr(-3) == '#0#') {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//编码
					_row.v2 = tdArr.eq(3).find('input').val();//名称
					_row.v3 = tdArr.eq(4).find('input').val();//规格型号
					_row.v4 = tdArr.eq(5).find('select').val();//单位
					_row.v5 = tdArr.eq(6).find('input').val();//单价
					_row.v6 = tdArr.eq(7).find('input').val();//数量
					_row.v7 = tdArr.eq(8).find('input').val();//金额
					_list.push(_row);
				}
			}
			return _list;
		...
		getCurrentGridTable: function (tbid, id) {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_ProcurementDetails#' + id + '#') && tbid =='tb_ProcurementDetails') {
					...
					_list.push({ ControlID: 'v3', ControlValue: tdArr.eq(4).find('input').val(), ControlType: '1' });//规格型号
					_list.push({ ControlID: 'v4', ControlValue: tdArr.eq(5).find('select').val(), ControlType: '2' });//单位
					_list.push({ ControlID: 'v5', ControlValue: tdArr.eq(6).find('input').val(), ControlType: '1' });//单价
					_list.push({ ControlID: 'v6', ControlValue: tdArr.eq(7).find('input').val(), ControlType: '1' });//数量
					_list.push({ ControlID: 'v7', ControlValue: tdArr.eq(8).find('input').val(), ControlType: '1' });//金额
				}
			}
			return _list;
		},
		...
		foreachGridTable: function () {
			...
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_ProcurementDetails#')) {
					var _row = { };
					_row.flag = flag;
					_row.v1 = tdArr.eq(2).find('input').val();//编码
					_row.v2 = tdArr.eq(3).find('input').val();//名称
		...
		disabledGridTable: function (grid) {
			//控制子表工具栏按钮
			if (grid == 'tb_ProcurementDetails') {
				$('#del_tb_ProcurementDetails').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#del_tb_ProcurementDetails').removeAttr('lay-event');
				$('#add_tb_ProcurementDetails').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#add_tb_ProcurementDetails').removeAttr('lay-event');
				$('#edit_tb_ProcurementDetails').attr('class', 'layui-btn layui-btn-disabled layui-btn-xs');
				$('#edit_tb_ProcurementDetails').removeAttr('lay-event');
				$('#add_tb_ProcurementDetails').attr('disabled', 'true');
				$('#edit_tb_ProcurementDetails').attr('disabled', 'true');
				$('#del_tb_ProcurementDetails').attr('disabled', 'true');
			}
			var trList = $(".layui-table").find("tr");  //获取table下的所有tr
			for (var i = 0; i < trList.length; i++) {  //遍历所有的tr
				var tdArr = trList.eq(i).find("td"); //获取该tr下的所有td
				var flag = tdArr.eq(0).text();
				if (flag.startsWith('#tb_ProcurementDetails#') && grid =='tb_ProcurementDetails') {
					...
					tdArr.eq(6).find('input').attr('disabled', 'disabled');//单价
					tdArr.eq(7).find('input').attr('disabled', 'disabled');//数量
					tdArr.eq(8).find('input').attr('disabled', 'disabled');//金额
					tdArr.eq(9).find('button').attr('disabled', 'disabled');//右侧添加|删除按钮
	...
	//暴露接口
	exports('ProcurementOfGoods', api);
});