...
layui.define(["element", "jquery"], function (exports) {
     ...
     * @returns {exTab}
     */
    exTab.prototype.init = function (option) {
        var _this = this; //这个this就是代表exTab（因为prototype的原因）
    ...
    //生成左侧菜单
    var temp = "";
    exTab.prototype.navBar = function (strData) {
        var data;
     ...
     * @param ele 当前tab
     */
    exTab.prototype.positionTab = function (contEle, ele) {
        var superEle = $(".com-tab");//父级元素
    ...
    exTab.prototype.tab = function () {
        ...
        element.on("tab({0})".format(filter), function (data) {
            ...
            var elSuper = $(".com-tab"),//视窗元素
                elMove = $(".com-tab-title"),//移动的元素
                elTabs = $(".com-tab-title li"),//所有已存在的tab集合
    ...
    exTab.prototype.tabDelete = function () {
        var that = this;
        var filter = this.tabConfig.tabFilter;
        //`tabDelete(${filter})`
    ...
    //添加tab页
    exTab.prototype.tabAdd = function (_thisa) {
        var that = this;
        ...
        var thatTabNum = $('.com-tab-title > li').length;//当前已经打开的选项卡的数量
        var iframes = $(".com-tab-content iframe");
        ...
        if (_this.attr("target") == "_blank") {
            ...
            for (var i = 0; i < thatTabNum; i++) {
                if ($('.com-tab-title > li').eq(i).attr('lay-id') == tabId) {
    ...
    exTab.prototype.render = function (fun) {
        var _this = this;//data
        ...
        if (_this.tabConfig.url) {
            ...
            alert("您的菜单配置有误请查看菜单配置说明");
        }
    };

    //刷新当前tab页
    exTab.prototype.refresh = function (_this) {
        $(".com-tab-content .layui-show").find("iframe")[0].contentWindow.location.reload(true);
    };
     ...
     * 3：代表关闭所有标签页
     */
    exTab.prototype.tabClose = function (num) {
        num = num || 1;
        num = num * 1;//强制转换成数字
        var that = this;
        let openTabs = $('.com-tab-title > li strong[lay-id]').not('strong[is-close=false]'),//获取已打开的tab元素(除开不会被关闭的导航)
            thatLayID = $('.com-tab-title > li.layui-this strong[lay-id]').not('strong[is-close=false]').attr("lay-id") || '';//获取当前打开的tab元素ID(除开不会被关闭的导航)
     ...
     *      为leftmax:跑到最左边
     */
    exTab.prototype.navMove = function (moveId, that) {
        var superEle = $(".com-tab"),//父级元素
            contEle = $(".com-tab-title"),//tab内容存放的父元素
            superWidth = parseInt(superEle.width()) - 40 * 3,//可显示的宽度
    ...
    exports("exTab", function (option) {
        if (parent.objEXTab) {
            return parent.objEXTab;
        } else {
            return new exTab().init(option);
        }
    });
});