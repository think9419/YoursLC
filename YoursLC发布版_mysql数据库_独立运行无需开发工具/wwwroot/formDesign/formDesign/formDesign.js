layui.define(['form', 'jquery', 'layer', 'cascader', "exUtils", 'tags'], function (exports) {
    "use strict";
    ...
    var $ = layui.$
        ...
        //主模板
        , TPL_MAIN = ['<div id="tpl_main" class="tpl_main">从左侧拖入组件进行表单设计或点击快速生成按钮<br><br>如无表单设计文件，自动生成的Form页面将按照『一行两列』排列控件<br></div>'].join('')
        ...
        , Class = function (options) {
            ...
            setTimeout(function () {
                options = that.config;
                var data = GetJsonFromApi();//
                if (data != "no" && data != "err" && data != "[]") {
                    $('#json-code').val(data);
                    ...
                    layer.msg('未找到表单设计文件 - 自动生成的Form页面将按照『一行两列』排列控件');
                }
                layer.close(loading);
            }, 1000);
        };
    ...
    Class.prototype.lang = {
        index: "序号",
        tag: "组件类型",
        name: "<div class='layui-input-inline'><i class='layui-icon layui-icon-about' lay-tips='file|date|img|checkbox|editor只能选择特定类型指标' data-offset='4'></i>指标ID&nbsp;&nbsp;&nbsp;<a class='layui-btn layui-btn-warm layui-btn-xs' lay-submit lay-filter='detail' id='detail'>指标属性</a></div>",
        label: "标签文本",
        type: '表单类型',
        placeholder: '提示',
        _default: '默认值',
        ...
        options.master = new Sortable(document.getElementById(options.id), {
            ...
            onAdd: function (evt) {
                ...
                if (typeof id != 'undefined') {
                    ...
                    that.recursiveDelete(options.data, id);//删除原始元素
                }
            ...
            onUpdate: function (evt) {
                ...
                //    alert(evt.newIndex + "--" + evt.oldIndex);

                //  /*  that.reloadComponent();// 尝试解决排序占位 0921*/
                //}
            },
            onEnd: function (evt) {
                if (evt.to.id == options.id) {
                    ...
                    var difValue = newIndex - oldIndex;
                    //差值只相差1不会出现排序占位
                    if (difValue == -1 || difValue == 1) {
                        /*alert("" + newIndex + " " + oldIndex);*/
                    }
                    else {
                        /*alert("newIndex：" + newIndex + " oldIndex：" + oldIndex);*/
                        that.reloadComponent();//刷新  尝试解决排序占位 0921
                    }
                }
            }
        });
    ...
    Class.prototype.events = function () {
        ...
        $('body').on('click', '#formBuilder ul li', function (e) {
            ...
            try {
                ...
                that.reloadComponent();//这个为解决谷歌等浏览器点击标签没反应！！！后期还要修改
            } catch (error) {
        ...
        form.on('select(componentSelected)', function (data) {
            ...
            try {
                if (element.tag == 'grid') {
                    ...
                    //alert("右侧下拉选择的："+data.value);
                    //alert("grid的列数：" +length);
                    if (data.value > length) {
                        // 添加元素
                        for (var i = length; i < data.value; i++) {
        ...
        //选择了指标
        form.on('select(indexSelected)', function (data) {
            var field = $(data.elem).data('field')
                , element = that.recursiveFindElem(options.data, options.state.name);

            var _select_name = JSON.stringify(options.data, null, 4);

            var oldElement = that.recursiveFindElem(options.data, $('#select_name').val());
            if (oldElement != null) {
                if (oldElement.name == $('#select_name').val()) {
                    $('#select_name').val('_unselected');
                    form.render();
        ...
        // 删除表单设计
        $('body').on('click', '.layui-form-del', function (e) {
            layer.confirm('删除后Form页面将按照『一行两列』排列控件，确定要删除表单设计文件吗？', {
            }, function (index, layero) {
                $.ajax({
                    url: "/SysTable/TbSetUp/DelTbJson",
                    type: "post",
                    data: { tbid: $("#tbid").val() },
                    dataType: "json",
                    async: false,
        ...
        // 刷新
        $('body').on('click', '.layui-btn-refresh', function (e) {
            that.reloadComponent();// 尝试解决排序占位 0921
        })

        //加载并刷新
        $('body').on('click', '.layui-btn-refresh2', function (e) {
            location.reload();
        })
        ...
        //快速生成
        $('body').on('click', '#creatjson', function (e) {
            var _type = '1行' + $("#AutoType").val() + '列';
            if ($("#AutoType").val() == '') {
                layer.msg('未选择');
                return false;
            }
            layer.confirm('此操作将会覆盖设计器当前设计，是否确定继续？<br>生成后需点击保存按钮！', {
            }, function (index, layero) {
                var loading = layer.load('Loading...', {
                    shade: [0.1, '#fff']
                });
    ...
    Class.prototype.getPropertieDebris = function (lable, hide = '') {
        ...
        if (lable) {
            proHtml += '<label class="layui-form-label"  style="text-align: left;">' + lable + '</label>';
        }
        proHtml += LAY_INPUT_INLINE;
        return proHtml;
    }
        ...
        for (const key in data) {
            ...
            switch (key) {
                ...
                case 'placeholder': // 标签和描述占位符
                    ...
                    break;
                case 'name':
                    if (data.tag == "gridtable") {
                        var tbList = GetGridTb();
                        optionsHtml += '<select lay-filter="componentSelected"  data-field="name" id="select_name" name="select_name">';
                        optionsHtml += '<option value="_unselected"';
                        //if (_value == data.name) {
                        //    optionsHtml += 'selected';
                        //}
                        optionsHtml += '>==请选择子表==</option>';

                        for (var item in tbList) {
                            var _value = tbList[item].TbId;
                            var _Text = tbList[item].TbName;

                            optionsHtml += '<option value="' + _value + '"';
                            if (_value == data.name) {
                                optionsHtml += 'selected';
                            }
                            optionsHtml += '>' + _Text + '</option>';
                        }
                        optionsHtml += '</select>';
                        break;
                    }
                    else {
                        var indexList = GetTbIndex(data.tag);
                        if (data.tag == "input" || data.tag == "textarea" || data.tag == "radio" || data.tag == "checkbox" || data.tag == "select" || data.tag == "slider" || data.tag == "switch" || data.tag == "date" || data.tag == "rate" || data.tag == "editor" || data.tag == "img" || data.tag == "file") {
                            optionsHtml += '<select lay-filter="indexSelected"  data-field="name" id="select_name" name="select_name" lay-search="">';
                            optionsHtml += '<option value="_unselected"';
                            //if (_value == data.name) {
                            //    optionsHtml += 'selected';
                            //}
                            optionsHtml += '>==请选择指标==</option>';

                            for (var item in indexList) {
                                var _value = indexList[item].IndexId;
                                var _Text = indexList[item].IndexName;
                ...
                case 'verify':
                    ...
                    break;
                case 'text_align':
                    optionsHtml += '<select lay-filter="componentSelected" data-field="text_align" >';
                    var date_types = [
                        {
                            title: '左',
                            value: 'Left',
                    ...
                    for (var index in date_types) {
                        const element = date_types[index];
                        optionsHtml += '<option value="' + element.value + '"';
                        if (element.value == data.text_align) {
                            optionsHtml += 'selected';
                        }
                        optionsHtml += '>' + element.title + '</option>';
                    }
                ...
                case 'data_dateformat':
                    ...
                    var dateformat = ['yyyy-MM-dd', 'yyyy-MM-dd HH:mm:ss'];
                ...
                case 'options':
                    ...
                    if (data.tag == "tab") {
                        ...
                        for (const key in data.options) {
                            ...
                            optionsHtml += '<input class="layui-input" value="' + option.title + '"  style="width:80px">';
                            optionsHtml += '<input class="layui-input" value="' + option.value + '" style="width:50px">';
    ...
    Class.prototype.formOptionHeader = function (options = 'form-options') {
        ...
        _options += '<legend style="color:darkgrey">选项</legend>';
        _options += ' </fieldset>';
    ...
    Class.prototype.getToolsHtml = function () {
        ...
        /*  tools += '<i class="layui-icon layui-icon-screen-full"  title="移动"></i>';*/
        /* tools += '&nbsp;';*/
        tools += '<i class="layui-icon layui-icon-picture-fine" title="复制" ></i>';
        tools += '&nbsp;&nbsp;';
        tools += '<i class="layui-bg-red layui-icon layui-icon-delete" title="删除"></i>';
    ...
    Class.prototype.renderComponentItem = {
        img: {
            render: function (data) {
                ...
                html += '<img name="' + data.name + '"  src="/images/nonexistent.gif"  alt="点击上传图片" style="height: 125px; width: 125px;" ';
                html += '>';
                html += '</div></div>';
                return html;
            }
        ...
        gridtable: {
            render: function (data) {
                ...
                var tbHTML = GetGridTableHtml(data.name);
                html += tbHTML;
                html += '</div></div>';
                return html;
            }
        ...
        file: {
            render: function (data) {
                ...
                html += 'value="" placeholder="' + data.placeholder + '"';
        ...
        input: {
            render: function (data) {
                ...
                html += '<input type="' + data.type + '" name="' + data.name + '" class="layui-input"';
                html += 'value="' + data._default + '" placeholder="' + data.placeholder + '"';
        ...
        colorpicker: {
            render: function (data) {
                ...
                html += '<div lay-colorpicker="' + data.name + '" data-value="' + data._default + '"></div>';
                html += '</div></div>';
                return html;
            }
        },
        ...
        span: {
            render: function (data) {
                var html = getBeforeItem(data);
                html += '<div style="text-align: ' + data.text_align + '; vertical-align: middle;"> <span>' + data.textarea + '</span></div>';
                /* html += '<div > <span style="text-align: ' + data.text_align + '; vertical-align: middle;">' + data.textarea + '</span></div>';*/
                html += LAY_FORM_DIV;
                return html;
            }
        },
    ...
    Class.prototype.recursiveDelete = function (array, name) {
        ...
        for (let index = 0; index < array.length; index++) {
            ...
            if (typeof element.children != 'undefined'
                ...
                for (let i = 0; i < subElem.length; i++) {
                    if (subElem[i].children.length) {
                        that.recursiveDelete(subElem[i].children, name);//删除原始元素
                    }
                }
            }
        }
    ...
    //返回指标列表
    function GetTbIndex(tag) {
        var indexList;
        $.ajax({
            url: "/SysTable/TbDesign/GetIndexListByTbIDAndTag?tbid=" + $("#tbid").val() + "&tag=" + tag,
            type: "post",
            async: false,
            success: function (result) {
                indexList = result;
    ...
    function GetGridTb() {
        var gridList;
        $.ajax({
            url: "/SysTable/TbDesign/GetGridList?tbid=" + $("#tbid").val(),
            type: "post",
            async: false,
            success: function (result) {
                gridList = result;
    ...
    //保存
    function CreatJsonFile(json) {
        $.ajax({
            url: "/SysTable/TbDesign/CreatJsonFile",
            type: "post",
            data: { tbid: $("#tbid").val(), json: json },
            dataType: "json",
            async: false,
            success: function (result) {
                if (result.state == 'error') {
                    /* let options = { icon: 5, time: 10000, anim: 1, btn: ['关闭'] };*/
                    let options = { icon: 5, time: 10000, anim: 1, btn: ['关闭'], area: ['600px', 'auto'] };
                    layer.msg(result.message, options);//显示错误
                }
                else {
                    let options = { icon: 1, time: 2000, anim: 1 };
                    layer.msg(result.message, options);
                }
            }
        });
    };
    ...
    function GetJsonStr() {
        var jsonStr;
        $.ajax({
            url: "/SysTable/TbDesign/GetJsonStr",
            type: "post",
            data: { tbid: $("#tbid").val(), type: $("#AutoType").val(), nColumns: $("#NColumns").val() },
            dataType: "json",
            async: false,
            success: function (result) {
                jsonStr = result;
    ...
    //生成子表html
    function GetGridTableHtml(id) {
        var jsonStr;
        $.ajax({
            url: "/SysTable/TbDesign/GetGridTableHtml",
            type: "post",
            data: { tbid: id },
            dataType: "json",
            async: false,
    ...
    function GetJsonFromApi() {
        var jsonStr;
        $.ajax({
            url: "/SysTable/TbDesign/GetJsonFromApi",
            type: "post",
            data: { tbid: $("#tbid").val() },
            dataType: "json",
            async: false,
    ...
    form.on("submit(detail)", function (data) {
        var _id = $('#select_name').val();
        if (_id != null) {
            if (_id.toLowerCase().startsWith('in')) {
                layer.open({
                    type: 2,
                    title: "录入指标",
                    shade: 0.5,
                    area: ['100%', '100%'],
                    anim: 2,
                    content: "/SysTable/TbSetUp/EditMainIndex?tbid=" + $("#tbid").val() + "&indexid=" + _id + "&frm=tbjson"
                });
            }
            if (_id.toLowerCase().startsWith('tb_')) {
                layer.open({
                    type: 2,
                    title: "子表管理",
                    shade: 0.5,
                    area: ['100%', '100%'],
                    anim: 2,
                    content: "/SysTable/TableList/TbGrid?tbid=" + $("#tbid").val() + "&grid=" + _id + "&frm=tbjson"
                });
            }
        }
        else {
            layer.msg('布局辅助控件无对应指标');
        }
    });
